...
def test_scattered_handle_pickles_small_and_keeps_metadata():
...
def test_scattered_input_resolves_inside_tasks():
    distributed = pytest.importorskip("distributed")
...
def test_metadata_survives_on_the_handle():
    distributed = pytest.importorskip("distributed")
...
def test_scattered_transition_potential_matches_embedded():
    distributed = pytest.importorskip("distributed")
...
def test_scattering_shrinks_the_task_graph():
    distributed = pytest.importorskip("distributed")
    ...
    with config.set({"device": "cpu"}), distributed.LocalCluster(
        n_workers=1, processes=True, threads_per_worker=1, dashboard_address=None
    ) as cluster, distributed.Client(cluster) as client:
